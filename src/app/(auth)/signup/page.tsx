...
export default function SignupPage() {
  ...
  return (
    <div
      className={`min-h-screen font-sans flex transition-colors duration-500 ${isDark ? "bg-[#0A0A0A] text-white" : "bg-[#faf9f6] text-stone-900"}`}
    >
      {/* ================= LEFT COLUMN: SIGNUP FORM ================= */}
      <div className="w-full lg:w-1/2 flex flex-col p-6 sm:p-12 md:p-16 justify-center relative z-10 min-h-screen">
        
        {/* Header: Logo & Theme Toggle (Perfectly split left and right for mobile & desktop) */}
        <div className="absolute top-6 left-6 right-6 sm:top-10 sm:left-12 sm:right-12 flex justify-between items-center z-20">
        ...
        <div className="max-w-sm w-full mx-auto mt-0 lg:mt-10 relative z-10">
          <div className="mb-6">
            <h2
          ...
          {displayError && (
            <div
              className={`mb-5 p-3 rounded-xl text-sm font-semibold flex items-start gap-3 transition-colors ${isDark ? "bg-red-500/10 border border-red-500/20 text-red-400" : "bg-red-50 border border-red-200 text-red-600"}`}
            >
              <div
                className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${isDark ? "bg-red-500/20" : "bg-red-100"}`}
              >
                !
              </div>
              {displayError}
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                  <User
                    className={`h-4 w-4 transition-colors ${isDark ? "text-white/40" : "text-stone-400"}`}
                  />
                </div>
                <input
                  name="name"
            ...
            <button
              disabled={loading}
              className={`w-full py-3 sm:py-3.5 text-sm sm:text-base font-bold rounded-xl transition-all duration-300 disabled:opacity-70 flex items-center justify-center gap-2 mt-4 ${
                isDark
            ...
            >
              {loading ? (
                <span className="flex items-center gap-2">
                  <svg
                    className={`animate-spin h-4 w-4 ${isDark ? "text-black" : "text-white"}`}
          ...
          <form action={googleFormAction}>
            <button
              type="submit"
              className={`w-full py-3 text-sm sm:text-base font-bold rounded-xl transition-all flex items-center justify-center gap-3 border ${
                isDark
      ...
      >
        ...
        <div className="relative z-10 p-12 max-w-lg mt-auto mb-12 w-full transition-all duration-500">
          ...
          >
            ...
            <p
              className={`font-medium leading-relaxed transition-colors ${isDark ? "text-stone-600" : "text-white/80"}`}
            >
              Join thousands of students saving time. Upload your assignments,
              pay online, and skip the massive print shop queues forever.
            </p>
          </div>
        </div>
      </div>
