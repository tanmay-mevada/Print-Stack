...
import {
  ...
  BarChart3, ChevronDown, Clock, XCircle, Timer
} from 'lucide-react'
...
export default function ShopDashboardPage() {
  ...
  // When modal opens, default to 30 mins from NOW
  useEffect(() => {
    if (isPauseModalOpen) {
      const now = new Date()
      now.setMinutes(now.getMinutes() + 30)
      setReopenTime(now.toTimeString().slice(0, 5)) // Formats as "HH:mm"
      setPauseDuration(30)
    }
  }, [isPauseModalOpen])

  // If user types MINUTES, calculate the new exact Time
  const handleDurationChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const mins = parseInt(e.target.value)
    setPauseDuration(e.target.value) // Keep string in state so they can backspace
  ...
  // If user types TIME, calculate the Minutes difference
  const handleTimeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (timeVal) {
      ...
      // If the time they picked has already passed today, assume it's for tomorrow
      if (target < new Date()) {
            ...
            <div className="space-y-6">
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className={`block text-[10px] font-bold uppercase tracking-widest ${isDark ? 'text-white/50' : 'text-stone-500'}`}>
                    Duration (Minutes)
                  </label>

                  {/* MAGICAL TRANSLATOR TEXT: Only shows if >= 60 mins */}
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full rounded-xl border pl-10 pr-4 py-3.5 outline-none font-bold ${isDark ? 'bg-[#0A0A0A] border-white/10 text-white focus:border-white/30' : 'bg-stone-50 border-stone-200 text-stone-900 focus:border-stone-400'}`}
                  />
      ...
      <div className="p-6 sm:p-8 max-w-6xl mx-auto space-y-8 relative z-10">
        ...
        {!shop ? (
          ...
          <div className="space-y-8 sm:space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                <div className="relative w-full sm:w-auto z-50" ref={statusRef}>
                  ...
                  {statusMenuOpen && (
                    <div className={`absolute top-[calc(100%+8px)] right-0 w-full sm:w-[320px] border rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 ${isDark ? 'bg-[#111111] border-white/10' : 'bg-white border-stone-200'}`}>
                      ...
                      <button
                        onClick={() => { setStatusMenuOpen(false); setIsPauseModalOpen(true); }}
                        className={`w-full text-left p-4 text-sm font-bold transition-colors flex items-center gap-3 ${isDark ? 'hover:bg-white/5 text-yellow-500' : 'hover:bg-stone-50 text-yellow-600'}`}
                      >
            ...
            {/* ... (Keep your existing ACTIVE and COMPLETED order tables exactly the same) ... */}
