...
export default function Home() {
  ...
  const portalLink = userRole === "shopkeeper" || userRole === "admin" ? "/shop/dashboard" : "/student/dashboard";
        ...
        <div className="flex justify-between items-center px-6 py-4 max-w-7xl mx-auto">
          ...
          <a href="#home" className="flex items-center gap-3">
            <div className="w-9 h-9 flex items-center justify-center">
              <img src={isDark ? "/pblackx.png" : "/pwhitex.png"} alt="PrintStack Logo" className="w-9 h-9 object-contain" />
            </div>
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            {/* Auth Buttons / Profile Dropdown (Desktop Only) */}
              ...
              {!user ? (
                ...
                <div className="relative">
                  <button
                    ...
                    className={`w-10 h-10 border rounded-full flex items-center justify-center text-sm font-bold transition-all duration-300 ${isDark ? 'border-white/20 bg-[#111111] hover:bg-white/10 text-white' : 'border-stone-300 bg-white hover:bg-stone-100 text-stone-900 shadow-sm'}`}
                  >
     ...
     {/* ================= MONOCHROME POLYMORPHIC HERO ================= */}
      ...
      >
        ...
        <div className="relative z-10 text-center px-4 lg:px-6 max-w-5xl">
          ...
          {/* Top Badges / Tags (Grouped in a flex container) */}
          <div className="flex flex-wrap justify-center items-center gap-3 md:gap-4 mb-12 lg:mb-10">
            {/* Custom Welcome Message (Shown when logged in) */}
            {user && (
              <span className={`inline-flex items-center gap-2 px-5 py-2 rounded-full border text-xs md:text-sm font-bold shadow-sm backdrop-blur-md transition-colors animate-in fade-in slide-in-from-bottom-4 duration-700 ${isDark ? "bg-white/10 border-white/20 text-white" : "bg-black/5 border-black/10 text-stone-800"}`}>
      ...
      {/* ================= FEATURES GRID ================= */}
      <section id="features" className={`py-16 md:py-20 px-5 md:px-6 transition-colors duration-500 ${isDark ? "bg-[#0A0A0A]" : "bg-[#faf9f6]"}`}>
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-5 md:gap-6">

            <div className={`md:col-span-8 group rounded-2xl md:rounded-3xl p-6 md:p-8 border transition-all duration-500 overflow-hidden relative flex flex-col ${isDark ? "bg-[#111111] border-white/10 hover:border-white/30" : "bg-white border-black/10 hover:border-black/30"}`}>
              <div className={`absolute top-0 right-0 w-40 md:w-64 h-40 md:h-64 rounded-full blur-3xl -z-10 transition-transform group-hover:scale-150 duration-700 ${isDark ? "bg-white/5" : "bg-black/5"}`} />
              <div className="h-full flex flex-col justify-between min-h-[180px] md:min-h-[240px]">
                <div className={`w-12 h-12 md:w-14 md:h-14 border rounded-xl md:rounded-2xl flex items-center justify-center transition-all duration-500 group-hover:rotate-6 ${isDark ? "bg-white/10 border-white/10 group-hover:bg-white" : "bg-black/5 border-black/10 group-hover:bg-stone-900"}`}>
            ...
            <div className={`md:col-span-4 group rounded-2xl md:rounded-3xl p-6 md:p-8 text-white border relative overflow-hidden transition-colors duration-500 flex flex-col ${isDark ? "bg-[#1A1A1A] border-white/10" : "bg-stone-900 border-stone-800"}`}>
              <div className={`absolute -top-16 md:-top-20 -right-16 md:-right-20 w-40 md:w-64 h-40 md:h-64 rounded-full blur-3xl ${isDark ? "bg-white/5" : "bg-white/10"}`} />
              <div className="relative z-10 h-full flex flex-col justify-between min-h-[180px] md:min-h-[240px]">
                <div className={`w-12 h-12 md:w-14 md:h-14 border rounded-xl md:rounded-2xl flex items-center justify-center ${isDark ? "bg-white/5 border-white/10" : "bg-white/10 border-white/20"}`}>
            ...
            <div className={`md:col-span-4 rounded-2xl md:rounded-3xl p-6 md:p-8 hover:scale-[1.02] transition-all duration-500 border flex flex-col ${isDark ? "bg-white text-black border-transparent" : "bg-stone-900 text-white border-stone-800"}`}>
            ...
            <div className={`md:col-span-8 group rounded-2xl md:rounded-3xl p-6 md:p-8 border transition-all duration-500 cursor-pointer flex flex-col ${isDark ? "bg-[#111111] border-white/10 hover:bg-white" : "bg-white border-black/10 hover:bg-stone-900"}`}>
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 h-full min-h-[200px] md:min-h-[220px]">
                ...
                <div className={`hidden md:flex w-16 h-16 border rounded-full items-center justify-center transition-all duration-500 shrink-0 ${isDark ? "bg-white/10 border-white/20 group-hover:bg-black" : "bg-black/5 border-black/10 group-hover:bg-white"}`}>
                  <ChevronRight className={`w-8 h-8 transition-colors ${isDark ? "text-white" : "text-stone-900"}`} />
                </div>
              </div>
            </div>
      ...
      {/* ================= FOOTER ================= */}
      <footer className={`py-16 px-6 border-t transition-colors duration-500 ${isDark ? "bg-[#050505] border-white/10 text-white/60" : "bg-[#faf9f6] border-black/10 text-stone-600"}`}>
        <div className="max-w-7xl mx-auto grid md:grid-cols-4 gap-12 mb-16">
          <div className="space-y-6 md:col-span-2">
            <div className="flex items-center gap-3">
              <img src={isDark ? "/pblackx.png" : "/pwhitex.png"} alt="PrintStack Logo" className="w-7 h-7 object-contain" />
      ...
      {showTop && (
        <button
          onClick={scrollToTop}
          className={`fixed bottom-6 left-1/2 -translate-x-1/2 md:left-auto md:translate-x-0 md:right-6 z-50 transition-all duration-300 hover:scale-110 p-0 md:p-3 bg-transparent md:rounded-full ${isDark ? "text-white md:bg-white md:text-black md:hover:bg-gray-200" : "text-stone-900 md:bg-stone-900 md:text-white md:hover:bg-stone-800"}`}
        >
