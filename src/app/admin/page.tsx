            ...
            <tbody className={`divide-y text-sm font-bold ${isDark ? 'divide-white/10' : 'divide-stone-200'}`}>
              {lists.orders.map((order: any) => (
                <tr key={order.id} className={`transition-colors ${isDark ? 'hover:bg-white/5' : 'hover:bg-stone-50'}`}>
                  ...
                  <td className="p-5 text-right">
                    <span className={`px-2.5 py-1 text-[10px] rounded-md uppercase tracking-widest border ${
                      order.status === 'COMPLETED' ? (isDark ? 'bg-green-500/10 text-green-400 border-green-500/20' : 'bg-green-50 text-green-700 border-green-200') :
  ...
  return (
    <div className={`min-h-screen flex font-sans transition-colors duration-500 ${isDark ? 'bg-[#050505] text-white selection:bg-white/20' : 'bg-[#faf9f6] text-stone-900 selection:bg-stone-900/20'}`}>
          ...
          {navItems.map((item) => {
            ...
            return (
              <button 
                ...
                className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-xl font-bold text-sm tracking-widest uppercase transition-all ${
                  isActive 
      ...
      <main className="flex-1 h-screen overflow-y-auto p-8 sm:p-12 relative">
        ...
        {isDark && (
           <div className={`fixed top-0 right-0 w-[800px] h-[800px] rounded-full blur-[150px] opacity-20 pointer-events-none transition-colors duration-1000 ${
              activeTab === 'disputes' ? 'bg-red-500' : activeTab === 'dashboard' ? 'bg-indigo-500' : 'bg-white/5'
           }`} />
        )}
